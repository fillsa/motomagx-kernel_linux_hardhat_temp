		      ...
		      struct flowi *fl, struct dst_entry **dst_p)
{
	...
	for (i = 0; i < nx; i++) {
		...
		if (tunnel) {
			struct flowi fl_tunnel; // netu v 2.6
			struct inet6_ifaddr *ifa; // netu v 2.6

			ipv6_addr_copy(&fl_tunnel.fl6_dst, remote);
			ipv6_addr_copy(&fl_tunnel.fl6_src, local);

			ifa = ipv6_get_ifaddr(local, NULL, 0); // netu v 2.6
			if (ifa) { // netu v 2.6
				fl_tunnel.oif = ifa->idev->dev->ifindex; // netu v 2.6
				in6_ifa_put(ifa); // netu v 2.6
			} // netu v 2.6
