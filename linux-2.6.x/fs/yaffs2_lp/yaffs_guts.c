 ...
 *                        Added the checkpoint mount option
 * 12-15-2006   Motorola  Do not shred during mount
...
#if defined(CONFIG_MOT_FEAT_YAFFS_SHREDDER)
		if (markNAND &&
           (bi->blockState != YAFFS_BLOCK_STATE_COLLECTING) &&
            dev->isYaffs2 &&
           (dev->sMountOptions & YAFFS_MOUNT_SHRED) &&
		   ( ! dev->middleOfMounting )) {

			/* This is the YAFFS2 (large page) shredding. */
	
			void *kaddr;
            yaffs_ExtendedTags newTags;

			yaffs_ReadChunkWithTagsFromNAND(dev, chunkId, NULL, &newTags);

            /* If the page contains the Object Header, we don't shred the page
             * since the data of this page could be read later. If we read the
             * shredded page later, we could have the ECC errors on that page.
             * So we should not shred the Object Header page.
             */

			if (newTags.chunkId != 0 ) {

#ifdef MOT_FEAT_SHRED_DEBUG

            printk("Going to SHRED: chunkInNand=%d, eccres=%d chused=%d obj=%d "
                   "chunkId=%d byte=%d del=%d seq=%d\n",
                 (chunkId - dev->chunkOffset), newTags.eccResult,
				 newTags.chunkUsed, newTags.objectId,
                 newTags.chunkId, newTags.byteCount, newTags.chunkDeleted,
                 newTags.sequenceNumber);
#endif

			    kaddr = kmap_atomic (ZERO_PAGE(0), KM_USER0);

			    /* With the Large page case, we need keep the OOB data
                 * since theses OOB data are used during mount time.
                 */

			    yaffs_WriteChunkWithTagsToNAND(dev, chunkId, (__u8 *) kaddr, NULL);
 ...
 */
static
void yaffs_FillCacheRemainder(yaffs_ChunkCache *cache, yaffs_Device *dev)
{
	u8 *localBuffer;
	yaffs_Object *in = cache->object;
	int chunk = cache->chunkId;

	if (cache->contig != dev->nDataBytesPerChunk) {
		localBuffer = yaffs_GetTempBuffer(dev, __LINE__);
		yaffs_ReadChunkDataFromObject(in, chunk, localBuffer);
		memcpy(&cache->data[cache->contig],
			&localBuffer[cache->contig],
			dev->nDataBytesPerChunk - cache->contig );
...
#ifdef CONFIG_MOT_FEAT_YAFFS_PARSE_MOUNT_OPTIONS
	if (dev->sMountOptions & YAFFS_MOUNT_NOCHKPOINT) {
		dev->isCheckpointed = 0;
		return dev->isCheckpointed;
	}
#endif
		...
		if (nToCopy != dev->nDataBytesPerChunk) {
			...
			if (dev->nShortOpCaches > 0) {
				...
				if (!cache
					...
					cache->contig = 0;
					if (start != 0) {
					/* Don't do the read until we know we need it. */
						yaffs_ReadChunkDataFromObject(in, chunk,
									      cache->
									      data);
						cache->contig = dev->nDataBytesPerChunk;
				...
				if (cache) {
					...
					if (start > cache->contig)
					{
						...
						cache->contig = ((cache->contig > (start + nToCopy)) ? cache->contig : (start + nToCopy));
					}
#endif
