 ...
 * ----          ------         --------
 * 03/12/2007    Motorola       Change copy_pc_to_user and copy_user_to_pc.
 */
...
static inline void copy_pc_to_user(void *to, const void *from, size_t n)
{
    ...
    n -= odd;
    while (n) {
	put_user(__raw_readl(from), (int*)toc);
	fromc += 4; toc += 4; n -= 4;
    }
    while (odd--)
	put_user(readb(fromc++), toc++);
}
...
static inline void copy_user_to_pc(void *to, const void *from, size_t n)
{
    ...
    n -= odd;
    while (n) {
	get_user(l, (int *)fromc);
	__raw_writel(l, (int *)to);
	toc += 4; fromc += 4; n -= 4;
    }
    while (odd--) {
	get_user(c, fromc++);
