...
* @return   void
*/
unsigned long sah_Handle_Interrupt(
    sah_Execute_Status hw_status)
{
        ...
        if ( (hw_status & SAH_EXEC_DONE1_BIT ) ||
            ...
            if (current_entry == NULL) {
                ...
                if (hw_status == SAH_EXEC_ERROR1) {
                    ...
                    current_entry->error_status = sah_HW_Read_Error_Status();
                } else {
                    ...
                    if (hw_status == SAH_EXEC_DONE1_DONE2) {
                        ...
                        if (current_entry == NULL) {
#if defined(DIAG_DRV_INTERRUPT) && defined(DIAG_DURING_INTERRUPT)
                            LOG_KDIAG("Done1_Done2 Interrupt received with "
                                      "one entry on queue.");
#endif
                        } else {
                ...
                do {
                    ...
                    if (dar == 0) {
                        ...
                        if (current_entry != NULL) {
                            ...
                            if (sah_Find_With_State(SAH_STATE_ON_SAHARA)
                                == NULL) {
#endif
            ...
            if (hw_status == SAH_EXEC_FAULT) {
                 ...
                 * on SAHARA */
                os_lock_save_context(desc_queue_lock, lock_flags);
                previous_entry = sah_Find_With_State(SAH_STATE_ON_SAHARA);
                os_unlock_restore_context(desc_queue_lock, lock_flags);
                ...
                if (previous_entry) {
                    ...
                    previous_entry->status = SAH_STATE_OFF_SAHARA;
                ...
                if (hw_status == SAH_EXEC_BUSY) {
                    LOG_KDIAG("Got Sahara interrupt in Busy state");
                } else {
                    if (hw_status == SAH_EXEC_IDLE) {
                        LOG_KDIAG("Got Sahara interrupt in Idle state");
                    } else {
                        LOG_KDIAG("Got Sahara interrupt in unknown state");
                    }
                }
#endif
            }
...
unsigned long sah_Handle_Poll(sah_Head_Desc *entry)
{
    ...
    if (sah_dpm_flag) {
        ...
        if (
#ifndef SUBMIT_MULTIPLE_DARS
                (hw_status == SAH_EXEC_IDLE) ||
                (hw_status == SAH_EXEC_DONE1_BUSY2) || /* should not happen */
#endif
            ...
            if (hw_status == SAH_EXEC_ERROR1) {
            /* Gather extra diagnostic information */
            entry->fault_address = sah_HW_Read_Fault_Address();
            entry->current_dar = sah_HW_Read_CDAR();
            /* Read this register last - it clears the error */
            entry->error_status = sah_HW_Read_Error_Status();
            /* translate from SAHARA error status to fsl_shw return values */
            entry->result = sah_convert_error_status(entry->error_status);
#ifdef DIAG_DRV_STATUS
            sah_Log_Error(entry->desc.dma_addr, entry->error_status,
                          entry->fault_address);
#endif
            } else {
     ...
     * if they are from user mode, and release the page cache for user pages */
