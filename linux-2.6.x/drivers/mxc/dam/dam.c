 ...
 * Date         Author    Comment
 * ----------   --------  ---------------------------
 * 04/11/2006   Motorola  Added support for DAI testing 
 *
 */
...
int dam_select_mode(dam_port port, dam_mode the_mode)
{
	...
	ModifyRegister32(dam_mode_masq << dam_mode_shift,
		...
		if(the_mode == CE_bus_network_mode)
		{ 
		    ModifyRegister32(0xFFFFFFFF,
                             0x00071010,
                             _reg_DAM_CNMCR);
		}
...
/* See mxc27530evb_gpio.c to complete the workaround for SSI mxc91231 pass1 */
#endif
/*!
		     ...
		     struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	if (_IOC_TYPE(cmd) != DAM_IOCTL)
		return -ENOTTY;

	switch (cmd) {
	case DAM_CONFIG_SSI1_MC13783:
	...
	case DAM_CONFIG_SSI_NETWORK_MODE_MC13783:
		dam_reset_register(port_1);
		dam_reset_register(port_2);
		dam_reset_register(port_4);
		dam_reset_register(port_5);
		...
		dam_select_TxClk_source(port_2, false, port_1);
		dam_select_TxFS_source(port_2, false, port_1);
...
static int __init dam_init(void)
{
	...
	printk("dam : creating devfs entry for DAM \n");
	devfs_mk_cdev(MKDEV(major_dam, 0),
		      S_IFCHR | S_IRUGO | S_IWUSR, DAM_NAME);
#endif
	return 0;
}
