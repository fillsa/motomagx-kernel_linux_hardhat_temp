...
wlan_process_rx_command(wlan_private * priv)
{
    ...
    if (Adapter->CurCmd->CmdFlags & CMD_F_HOSTCMD) {
        ...
        case Host_RET_802_11_IBSS_BCN_MONITOR:
            {
                HostCmd_DS_802_11_IBSS_BCN_MONITOR *pstIbss_bcn_monpd;

                pstIbss_bcn_monpd = &resp->params.ibss_bcn_monpd;
                pstIbss_bcn_monpd->Action =
                    wlan_le16_to_cpu(pstIbss_bcn_monpd->Action);

                if (pstIbss_bcn_monpd->Action == HostCmd_ACT_GEN_GET) {
                    pstIbss_bcn_monpd->u32BcnMonpd =
                        wlan_le32_to_cpu(pstIbss_bcn_monpd->u32BcnMonpd);
                    memmove(Adapter->CurCmd->pdata_buf, pstIbss_bcn_monpd,
                            sizeof(HostCmd_DS_802_11_IBSS_BCN_MONITOR));
                }
                break;
            }
        ...
        case HostCmd_RET_INACTIVITY_TIMEOUT_EXT:
            memcpy(Adapter->CurCmd->pdata_buf,
                   &resp->params.inactivityext,
                   sizeof(HostCmd_DS_INACTIVITY_TIMEOUT_EXT));
            break;
        case HostCmd_RET_DBGS_CFG:
            break;
        case HostCmd_RET_GET_MEM:
            {
                FW_MEM_DATA *pFwData =
