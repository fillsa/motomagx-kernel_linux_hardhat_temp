...
static void disable_overlays(struct pxafb_info *fbi)
{
	if (directfb)
	{
		/* Save all overlays registers ... */
		fbi->reg_ovl1c2 = OVL1C2;
		fbi->reg_ovl1c1 = OVL1C1;
	        fbi->reg_ovl2c2 = OVL2C2;
	        fbi->reg_ovl2c1 = OVL2C1;

		fbi->reg_fdadr1 = FDADR1 & ~0xf;
		fbi->reg_fdadr2 = FDADR2 & ~0xf;
		fbi->reg_fdadr3 = FDADR3 & ~0xf;
		fbi->reg_fdadr4 = FDADR4 & ~0xf;
...
static void enable_overlays(struct pxafb_info *fbi)
{
	if (directfb)
	{
		u_int	ovl1_en = fbi->reg_ovl1c1 & OVL1C1_O1EN;
		u_int   ovl2_en = fbi->reg_ovl2c1 & OVL2C1_O2EN;
		u_int	ovl2_fmt = fbi->reg_ovl2c1 >> 20;

		/* disable branch/start/end of frame interrupts */
		LCCR5 |= ( LCCR5_IUM1 | LCCR5_BSM1 | LCCR5_EOFM1 | LCCR5_SOFM1 |
			LCCR5_IUM2 | LCCR5_BSM2 | LCCR5_EOFM2 | LCCR5_SOFM2 |
			LCCR5_IUM3 | LCCR5_BSM3 | LCCR5_EOFM3 | LCCR5_SOFM3 |
			LCCR5_IUM4 | LCCR5_BSM4 | LCCR5_EOFM4 | LCCR5_SOFM4);

		/* restore overlays control registers.
		 * don't enable overlays here */
		OVL1C2 = fbi->reg_ovl1c2;
		OVL1C1 = fbi->reg_ovl1c1 & ~OVL1C1_O1EN;
		OVL2C2 = fbi->reg_ovl2c2;
	        OVL2C1 = fbi->reg_ovl2c1 & ~OVL2C1_O2EN;

		if (ovl1_en)
		{
			FDADR1 = fbi->reg_fdadr1;
		}

		if (ovl2_en)
		{
			FDADR2 = fbi->reg_fdadr2;

			/* in case of YUV mode restore fdadr3 and fdadr4*/
			if (ovl2_fmt != 0)
			{
	                        FDADR3 = fbi->reg_fdadr3;
	                        FDADR4 = fbi->reg_fdadr4;
			}
		}

		/* Enable overlays only if they was enabled before
		 * overlays_disable() call */
		OVL1C1 |= ovl1_en;
		OVL2C1 |= ovl2_en;
		return;
	}
...
#ifdef CONFIG_DPM
static int pxafb_scale(struct notifier_block *self, unsigned long level,
	void *newop)
{
	if(pxafbi != NULL){
		u_int pcd;

		/* set_ctrlr_state:  C_DISABLE_CLKCHANGE
		 *
		 * do the same as pxafb_disable_controller() does,
		 * but no sleep
		 */
		LCSR = 0xffffffff;	/* Clear LCD Status Register */
		LCCR0 &= ~LCCR0_LDM;	/* Enable LCD Disable Done Interrupt */
		LCCR0 |= LCCR0_DIS;	/* Disable LCD Controller */
		WAIT_FOR_LCD_INTR(LCSR0,LCSR_LDD,20);
#ifdef CONFIG_PXA27x
		disable_overlays(pxafbi);
#endif
		pcd = get_pcd(pxafbi->fb.var.pixclock);
		pxafbi->reg_lccr3 = (pxafbi->reg_lccr3 & ~0xff) | LCCR3_PixClkDiv(pcd);
		/* set_ctrlr_state:  C_ENABLE_CLKCHANGE */
		pxafb_enable_controller(pxafbi);
#ifdef  CONFIG_PXA27x
		enable_overlays(pxafbi);
#endif
	}
	return 0;
}
...
#if defined(CONFIG_DPM) && defined(CONFIG_MACH_MAINSTONE)
	pdev->dev.constraints=&pxafb_constraints;
