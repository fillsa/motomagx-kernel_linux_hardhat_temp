...
/**
 * Update the gpio_signal_mapping table based on information from the HWCFG
 * tree.
 *
 * @return 0 on success; less than zero on failure.
 */
...
{
    if(GPIO_SIGNAL_IS_VALID(index)) {
        gpio_free_irq_dev(gpio_signal_mapping[index].port,
                gpio_signal_mapping[index].sig_no, prio, dev_id);
        return 0;
    }
...
/*
 * Export gpio_signal_mapping array for GPIODev.
 */
