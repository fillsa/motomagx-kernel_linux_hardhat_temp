...
trace_event:
	...
	if (ltt_test_bit(event_id, &trace->log_event_details_mask)) {
		...
		case LTT_EV_DEFINE_NAME:
			...
			break;
		case LTT_EV_DPM:
			if (((ltt_dpm *) event_struct)->event_sub_id == LTT_EV_DPM_OP) {
				var_data_beg = ((ltt_dpm *) event_struct)->event_name;
				var_data_len = ((ltt_dpm *) event_struct)->event_data1;
				data_size += (uint16_t) var_data_len;
			}
			break;
		}
...
#endif
#ifdef CONFIG_LTT_DPM
	ltt_set_bit(LTT_EV_DPM, &trace->traced_events);
	ltt_set_bit(LTT_EV_DPM, &trace->log_event_details_mask);
#endif
}
