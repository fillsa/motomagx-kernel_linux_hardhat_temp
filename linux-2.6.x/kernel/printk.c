...
asmlinkage int vprintk(const char *fmt, va_list args)
{
	...
	for (p = printk_buf; *p; p++) {
		if (log_level_unknown) {
#ifndef CONFIG_MOT_FEAT_PRINTK_TIMES// non 2.6.12
			...
			if (printk_time) {
				#define TBUF_LEN 19 // 2.6.12  TBUF_LEN 50
				int loglev_char;
...
{
	static DEFINE_RAW_SPINLOCK(ratelimit_lock); //2.6.12 +	static DEFINE_SPINLOCK(ratelimit_lock);
