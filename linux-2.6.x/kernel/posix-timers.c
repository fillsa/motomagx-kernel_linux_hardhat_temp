...
common_timer_get(struct k_itimer *timr, struct itimerspec *cur_setting)
{
	...
	while ((volatile long) (timr->it.real.timer.expires) != expire.jiffies);
	...
	if (timeleft &&
		...
		expire = zero_now; //2.6.12+		timr->it.real.timer.expires = expires = 0;
	}
	if (timeleft) {
		if (timr->it_requeue_pending & REQUEUE_PENDING ||
			...
			if (!timer_pending(&timr->it.real.timer))
				expire = zero_now; // 2.6.12 				expires = 0;
		if (timeleft) {
			sub_now(&expire, &now);
		}
	}
			   ...
			   int *err)
{
	...
	if (abs) {
		...
		if ((clock - &posix_clocks[0]) <= CLOCK_MONOTONIC_HR) {
			 ...
			 */
			if(clock->clock_get == posix_clocks[CLOCK_MONOTONIC].clock_get){
...
/*2,6,12		set_normalized_timespec(&oc, oc.tv_sec,
		 ...
		 struct itimerspec *new_setting, struct itimerspec *old_setting)
{
	...
	IF_HIGH_RES(arch_cycle_expire = )
		adjust_abs_time(clock,
			    ...
			    &expire_64, &(timr->it.real.wall_to_prev), &rtn);
	/*
